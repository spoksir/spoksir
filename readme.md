<div align="center">   
  <p id="title">
   <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&width=435&lines=Ol%C3%A1%2C+eu+sou+Francisco+Vieira"/>
  </p>
  <p id="subtitle">
    𝗤𝗔 | Analyst | Automation
  </p>    
  <br>
</div>


## 📋About me
  ISTQB-Certified Tester with strong manual testing expertise (test planning, test-case development, defect tracking) and hands-on experience learning automation tools. Currently pursuing a Bachelor’s in Informatics Engineering and transitioning from physical security and customer support, I bring a security-minded, detail-oriented approach, proven problem-solving abilities, and a commitment to delivering high-quality software.


## 🌎 Find me
- 📚 [Curriculum](curriculum.md)
- 🐞 [Portfólio QA](https://github.com/spoksir)
- 💼 [LinkedIn](https://www.linkedin.com/in/fmlvieira/)
- ✉️ [Email](mailto:franciscovieira88@gmail.com)
- <img alt="Discord" height="20" width="20" src="https://github.com/gui-bus/TechIcons/blob/main/Dark/Discord.svg" /> Discord (maviefra)


## 💻 Experience
  Security & Customer Care | Grupo 8:
  - I was responsible for security and quality at locations such as Makro, Pingo Doce, Luís Simões Transportes, Constantino Nery Theatre, university residences, Auchan, parking facilities, and shopping centres.
Conducted daily compliance audits and documented incidents with photo/video evidence,  status, and follow-up deadlines, Tracked non-conformities in logs, alerted responsible teams and updated records post-resolution, Managed digital records, maintaining data integrity, Verified CCTV/access control functionality and reported system failures promptly.


 
## 🎓 Education
  - Bachelor’s degree in Computer Engineering | UAb - Current (expected 2027)
  - Cybersecurity Training Program (Level 5 QNQ) | IEFP


## 📝 Certification
  - ISTQB – Certified Tester Foundation | PTSQB Portuguese Software Testing Association



## 👨‍💻SKILLS

<details>
  <summary>Elaboração de Documentos da Qualidade</summary>

  - Plano de Testes
  - Cenários e Casos de Teste
  - Relatório de Bugs
  - Resultados dos Testes
</details>

<details>
  <summary>Metodologias Ágeis</summary>

  - Kanban
  - Scrum
</details>


<details>
  <summary>Test Automation</summary>

  - Robot Framework
  - Cypress
  - Cucumber
  - Postman
  - Maestro 
</details>


<details>
  <summary>Test Management</summary>

  - Jira
  - Testrail

</details>



<details>
  <summary>CI / CD</summary>

  - Git
  - Github
  - Gitlab
</details>

<details>
  <summary>Linguagem de Programação</summary>

  - Javascript
  - Python
</details>

<details>
  <summary>Data bases</summary>

  - SQL
  - DBeaver
  - Power BI
</details>

<br>

